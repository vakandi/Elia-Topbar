...
@MainActor final class RunPopupController {
    ...
    func show(for agentName: String, dropX: CGFloat, duration: TimeInterval) {
        ...
        if panels[agentName] != nil { logPopup("show already has panel for \(agentName) reschedule"); scheduleRetract(for: agentName, after: duration); return }
        ...
        panel.contentView = NSHostingView(rootView: RunPopupView(agentName: agentName) { [weak self] in self?.retract(for: agentName) })
        ...
        scheduleRetract(for: agentName, after: duration)
    }
    ...
    private func close(for name: String, panel: NSPanel) { panel.orderOut(nil); panels[name]=nil; hoverStates[name]=nil; durations[name]=nil }
    ...
    func setHover(_ hovering: Bool, for agentName: String) {
        ...
        else { scheduleRetract(for: agentName, after:1.5) }
    }
}
...
struct RunPopupView: View {
    ...
    var body: some View {
        HStack(spacing:0){
            ...
            VStack(spacing:6){ photoBadge; bubble }.padding(.top,4).frame(width: 264)
        }
    ...
    private var bubble: some View {
        VStack(alignment:.leading,spacing:4){
            HStack(spacing:6){ Circle().fill(Color.green).frame(width:6,height:6); Text("\(agentName) running").font(.caption).fontWeight(.semibold).foregroundColor(.secondary); Spacer(); Text("live").font(.caption2).foregroundColor(.orange) }
        ...
        }.padding(10).frame(width:264).background(RoundedRectangle(cornerRadius:14).fill(.regularMaterial)).overlay(RoundedRectangle(cornerRadius:14).stroke(Color.primary.opacity(0.12)))
    }
